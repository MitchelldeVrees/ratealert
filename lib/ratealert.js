const API_URL = "https://www.independer.nl/api/spaarrekening/zoekresultaat/getzoekresultaat?v=61";
...
async function fetchOffers(apiUrl = API_URL) {
  const res = await fetch(apiUrl, {
    headers: { "User-Agent": "RateRadar/0.2" },
  });
  if (!res.ok) {
...
function buildRankings(offers, principal, horizons, topN) {
  const rankings = {};
